  ...
  <div class="panel" :class="className">
    <div v-if="searchInput" class="p-3">
      <div class="input-group input-group-sm">
        <router-link v-if="txtNew" :to="{ name: linkName, params: { id : '' } }"
                     class="bg-green-700 border border-transparent hover:bg-green-800 text-white font-bold py-1 px-2 text-xs rounded-l">
          <i class="fa fa-plus"/>
          {{ txtNew }}
        </router-link>
        <input type="text" class="py-1 px-2 text-xs border-0 border-t border-b border-gray-300" :placeholder="$store.getters['Lang/get']('element_filter_msg')"
               @keyup="filter">
        <a v-if="txtHelp" href="javascript:;"
           ...
           @click="msg=!msg">{{ $store.getters['Lang/get']('help') }}</a>
      </div>
      ...
      <template v-for="category in data">
        <li :key="'category-' + category.id" v-if="Object.values(category.items).filter(v => !v.hidden).length">
          ...
          <a v-if="!hiddenCategories"
             class="px-3 py-2 bg-secondary bg-opacity-10 border-top border-bottom text-decoration-none text-muted">
